...
use crate::client::queries::{
    ProjectMilestoneCreateInput as QueryCreateInput, ProjectMilestoneCreateMutation,
    ProjectMilestoneCreateMutationVariables, ProjectMilestoneDeleteMutation,
    ProjectMilestoneDeleteMutationVariables, ProjectMilestoneQuery, ProjectMilestoneQueryVariables,
    ProjectMilestoneUpdateInput as QueryUpdateInput, ProjectMilestoneUpdateMutation,
    ProjectMilestoneUpdateMutationVariables, ProjectMilestonesForProjectQuery,
    ProjectMilestonesForProjectQueryVariables, ProjectMilestonesQuery,
    ProjectMilestonesQueryVariables, TimelessDate,
};
...
impl MilestoneClient for LinearClient {
    fn get_milestone(&self, token: &str, id: &str) -> Result<Milestone, CliError> {
        ...
        let response = self.execute_query(
            token,
            ProjectMilestoneQuery::build(ProjectMilestoneQueryVariables { id: id.to_string() }),
            crate::client::GraphQlErrorType::General,
        )?;
        Ok(response
            .data
            .ok_or_else(|| CliError::NotFound(format!("Milestone {id} not found")))?
            .project_milestone
            .into())
    }
    ...
    fn list_milestones(
        ...
        let nodes = if let Some(project_id) = project_id {
            self.execute_query(
                token,
                ProjectMilestonesForProjectQuery::build(
                    ProjectMilestonesForProjectQueryVariables {
            ...
            self.execute_query(
                token,
                ProjectMilestonesQuery::build(ProjectMilestonesQueryVariables { first, name }),
                crate::client::GraphQlErrorType::General,
            )?
            .data
            .ok_or_else(|| CliError::General("No data returned".to_string()))?
        ...
        let response = self.execute_query(
            ...
            ProjectMilestoneCreateMutation::build(ProjectMilestoneCreateMutationVariables {
                input: QueryCreateInput {
                    ...
                    target_date: input.target_date.map(TimelessDate),
                },
            }),
            crate::client::GraphQlErrorType::General,
        )?;
