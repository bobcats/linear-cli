...
use crate::output::{
    JsonStyle, OutputFormat, get_format_with_provider, resolve_json_style_with_provider,
};
...
pub fn handle_delete(
    reference: &str,
    project: Option<&str>,
    client: &dyn MilestoneClient,
    lookup: &dyn MilestoneReferenceLookup,
    ...
    let id = resolver.resolve_required_id(
        token.expose_secret(),
        reference,
    ...
    if matches!(
        get_format_with_provider(format_flag, config),
        OutputFormat::Json
    ) {
        ...
        let output = match resolve_json_style_with_provider(config) {
            JsonStyle::Compact => serde_json::to_vec(&payload),
            JsonStyle::Pretty => serde_json::to_vec_pretty(&payload),
        }
