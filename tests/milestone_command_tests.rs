...
fn sample_milestone() -> Milestone {
    Milestone {
        ...
        project: MilestoneProject { id: "project-1".to_string(), name: "App".to_string(), slug_id: "app".to_string() },
...
fn milestone_list_prints_json_from_client() {
    ...
    handle_list(None, 10, &client, &config_with_token(), &TestStorage, &io, Some(OutputFormat::Json)).unwrap();
...
fn milestone_view_prints_one_milestone() {
    ...
    handle_view("milestone-1", &client, &config_with_token(), &TestStorage, &io, Some(OutputFormat::Table)).unwrap();
...
fn milestone_create_passes_input_to_client() {
    ...
    handle_create("project-1", "Beta", Some("Beta readiness".to_string()), Some("2026-06-30".to_string()), &client, &config_with_token(), &TestStorage, &io, Some(OutputFormat::Json)).unwrap();
...
fn milestone_update_requires_patch_field() {
    ...
    let result = handle_update("milestone-1", UpdateMilestoneInput::default(), &client, &config_with_token(), &TestStorage, &io, Some(OutputFormat::Json));
...
fn milestone_delete_prints_json_success() {
    ...
    handle_delete("milestone-1", &client, &config_with_token(), &TestStorage, &io, Some(OutputFormat::Json)).unwrap();
